package cn.langlang.javanter.interpreter.evaluator;
...
import cn.langlang.javanter.runtime.nativesupport.StaticImportRegistry;
import cn.langlang.javanter.runtime.nativesupport.TypeRegistry;
import java.util.*;
...
public class ExpressionEvaluator extends AbstractASTVisitor<Object> {
    ...
    private void collectUsedVariables(ASTNode node, Set<String> variables) {
        ...
        } else if (node instanceof cn.langlang.javanter.ast.statement.IfStatement) {
            cn.langlang.javanter.ast.statement.IfStatement ifStmt = (cn.langlang.javanter.ast.statement.IfStatement) node;
        ...
        } else if (node instanceof cn.langlang.javanter.ast.statement.WhileStatement) {
            cn.langlang.javanter.ast.statement.WhileStatement whileStmt = (cn.langlang.javanter.ast.statement.WhileStatement) node;
            collectUsedVariables(whileStmt.getCondition(), variables);
            collectUsedVariables(whileStmt.getBody(), variables);
        } else if (node instanceof cn.langlang.javanter.ast.statement.ForStatement) {
            cn.langlang.javanter.ast.statement.ForStatement forStmt = (cn.langlang.javanter.ast.statement.ForStatement) node;
        ...
        } else if (node instanceof cn.langlang.javanter.ast.statement.ReturnStatement) {
            cn.langlang.javanter.ast.statement.ReturnStatement ret = (cn.langlang.javanter.ast.statement.ReturnStatement) node;
            collectUsedVariables(ret.getExpression(), variables);
        } else if (node instanceof cn.langlang.javanter.ast.statement.ExpressionStatement) {
            cn.langlang.javanter.ast.statement.ExpressionStatement exprStmt = (cn.langlang.javanter.ast.statement.ExpressionStatement) node;
            collectUsedVariables(exprStmt.getExpression(), variables);
        } else if (node instanceof cn.langlang.javanter.ast.statement.TryStatement) {
            cn.langlang.javanter.ast.statement.TryStatement tryStmt = (cn.langlang.javanter.ast.statement.TryStatement) node;
            collectUsedVariables(tryStmt.getTryBlock(), variables);
            for (cn.langlang.javanter.ast.misc.CatchClause catchClause : tryStmt.getCatchClauses()) {
                collectUsedVariables(catchClause.getBody(), variables);
            }
            collectUsedVariables(tryStmt.getFinallyBlock(), variables);
        } else if (node instanceof cn.langlang.javanter.ast.statement.ThrowStatement) {
            cn.langlang.javanter.ast.statement.ThrowStatement throwStmt = (cn.langlang.javanter.ast.statement.ThrowStatement) node;
        ...
        } else if (node instanceof cn.langlang.javanter.ast.statement.SynchronizedStatement) {
            cn.langlang.javanter.ast.statement.SynchronizedStatement sync = (cn.langlang.javanter.ast.statement.SynchronizedStatement) node;
    ...
    private ScriptClass createRunnableInterface() {
        ScriptClass runnableInterface = new ScriptClass(
            "Runnable", "java.lang.Runnable", cn.langlang.javanter.parser.Modifier.ABSTRACT | cn.langlang.javanter.parser.Modifier.INTERFACE,
        ...
        ScriptMethod runMethod = new ScriptMethod(
            "run", cn.langlang.javanter.parser.Modifier.PUBLIC | cn.langlang.javanter.parser.Modifier.ABSTRACT, voidType,
    ...
    private ScriptClass createInterfaceFromClass(Class<?> clazz) {
        ScriptClass iface = new ScriptClass(
            clazz.getSimpleName(), clazz.getName(), cn.langlang.javanter.parser.Modifier.INTERFACE,
            null, new ArrayList<>(), null
        );
        ...
        for (java.lang.reflect.Method method : clazz.getMethods()) {
            ...
            for (java.lang.reflect.Parameter param : reflectParams) {
                cn.langlang.javanter.ast.type.Type paramType = createTypeFromJavaType(param.getType());
                params.add(new ParameterDeclaration(null, 0, paramType, param.getName(), 
                    false, new ArrayList<>()));
            }
            
            cn.langlang.javanter.ast.type.Type returnType = createTypeFromJavaType(method.getReturnType());
            ScriptMethod scriptMethod = new ScriptMethod(
                method.getName(), cn.langlang.javanter.parser.Modifier.PUBLIC | cn.langlang.javanter.parser.Modifier.ABSTRACT, returnType,
    ...
    private cn.langlang.javanter.ast.type.Type createTypeFromJavaType(Class<?> type) {
        String typeName;
        if (type.isPrimitive()) {
            typeName = type.getName();
        } else if (type.isArray()) {
            Class<?> componentType = type.getComponentType();
            cn.langlang.javanter.ast.type.Type component = createTypeFromJavaType(componentType);
            return new cn.langlang.javanter.ast.type.Type(null, component.getName(), 
                component.getTypeArguments(), component.getArrayDimensions() + 1, new ArrayList<>());
        } else {
            typeName = type.getSimpleName();
        }
        return new cn.langlang.javanter.ast.type.Type(null, typeName, new ArrayList<>(), 0, new ArrayList<>());
    }
    ...
    @Override
    public Object visitTypeParameter(cn.langlang.javanter.ast.type.TypeParameter node) {
        return null;
    }
    
    @Override
    public Object visitTypeArgument(cn.langlang.javanter.ast.type.TypeArgument node) {
        return null;
    }
